...
function autoScrollNearViewportEdge(scroller, clientY, edgeZone = 88, maxSpeed = 22) {
...
function decideEnterMobileSelectionMode(context, e) {
  if (!context.isMobileEnvironment)
    return { type: "none" };
...
function decideEnterMobileSelectionModeFromBlock(context, blockInfo, e) {
  ...
  if (!context.isMultiLineSelectionEnabled)
    return { type: "none" };
  if (!context.canStartDragForPointer("touch", "text"))
    return { type: "none" };
...
function resolvePointerInputPolicy(context, e) {
...
function decideSelectionResize(context, e, target, policy) {
  if (!policy.canResizeSelection)
    return { type: "none" };
...
function decidePassiveSelectionDrag(context, e, target) {
  ...
  if (pointerType === "mouse" && selectedHandleHit)
    return { type: "none" };
  ...
  if (!context.canStartDragForPointer(pointerType, sourceKind))
    return { type: "none" };
  if (context.pipelineState.type === "selecting" && context.hasActiveRangePointerSession) {
    return { type: "retarget_mobile_range_selection" };
  }
  const longPressMs = pointerType !== "mouse" && context.isMobileEnvironment ? MOBILE_SELECTED_RANGE_DRAG_LONG_PRESS_MS : void 0;
  return {
...
function decideHandlePointerDown(context, e, handle, policy) {
  if (e.button !== 0)
    return { type: "none" };
...
function resolveHandleRangeSelectionPolicy(context, e, policy) {
  if (!context.isMultiLineSelectionEnabled)
    return null;
  if (policy.platform === "mobile") {
    if (isPassiveSelectionActive(context))
...
function decideRangeSelectionFromHandleWhileSelecting(context, handle, e) {
  if (context.pipelineState.type !== "selecting" || context.pipelineState.selection.phase !== "passive")
    return { type: "none" };
...
function decideTextRangeSelection(context, e, target, policy) {
  if (!isPassiveSelectionActive(context))
    return { type: "none" };
  if (!policy.canUseTextLongPress)
    return { type: "none" };
  if (!shouldStartMobilePressDrag(e))
    return { type: "none" };
  if (!context.canStartDragForPointer(e.pointerType || null, "text"))
    return { type: "none" };
...
function decideRetargetRangeSelectionFromHandleWhileSelecting(context, handle, e) {
  if (context.pipelineState.type !== "selecting")
    return { type: "none" };
  if (e.pointerType === "mouse")
    return { type: "none" };
...
function decideTextLongPressDrag(context, e, target, policy) {
  if (!policy.canUseTextLongPress)
    return { type: "none" };
...
var RangeSelectionBoundaryHandleRenderer = class {
  ...
  render(blocks, options) {
    if (!(options == null ? void 0 : options.showMobileResizeHandles) || !this.isMobileEnvironment()) {
      this.clear();
      return;
    }
...
function handleRangeSelectionPointerMove(host, e, state) {
  ...
  e.preventDefault();
  e.stopPropagation();
  const targetBoundary = (_b = resolveHandleRangeBoundaryAtPoint(host, e.clientX, e.clientY)) != null ? _b : resolveRangeBoundaryAtPoint(host.view, e.clientX, e.clientY, (x, y) => {
    var _a2, _b2;
    return (_b2 = (_a2 = host.resolveBlockSelection({ kind: "point", clientX: x, clientY: y })) == null ? void 0 : _a2.anchorBlock) != null ? _b2 : null;
  });
  if (targetBoundary) {
  ...
  autoScrollEditorNearViewportEdge(
    host.view,
    e.clientY,
    (_d = (_c = host.deps).getAutoScrollEdgeZonePx) == null ? void 0 : _d.call(_c),
    (_f = (_e = host.deps).getAutoScrollMaxSpeedPx) == null ? void 0 : _f.call(_e)
  );
}
function resolveHandleRangeBoundaryAtPoint(host, clientX, clientY) {
  if (typeof activeDocument.elementFromPoint !== "function") {
    return null;
  }
  const hit = activeDocument.elementFromPoint(clientX, clientY);
  if (!(hit == null ? void 0 : hit.instanceOf(HTMLElement)))
    return null;
  const handle = hit.closest(`.${DRAG_HANDLE_CLASS}`);
  if (!handle || handle.classList.contains(EMBED_HANDLE_CLASS))
    return null;
  if (!host.view.dom.contains(handle))
    return null;
...
function finishRangeSelectingPointer(host, e, mode) {
  ...
  if (rangeState.pointerId !== -1 && e.pointerId !== rangeState.pointerId)
    return;
  if (mode === "up" && !rangeState.longPressReady && host.pipelineState.type === "idle" && shouldOpenBlockTypeMenuFromShortTextTap(host, e.pointerType || null, rangeState.sourceKind)) {
    e.preventDefault();
    e.stopPropagation();
...
function finishPressPendingPointer(host, e, mode) {
  ...
  if (e.pointerId !== pressState.pointerId)
    return;
  if (mode === "up" && !pressState.longPressReady && (host.pipelineState.type === "holding" || host.pipelineState.type === "ready_to_drag") && shouldOpenBlockTypeMenuFromShortTextTap(host, e.pointerType || null, host.pipelineState.hold.target.source)) {
    e.preventDefault();
    e.stopPropagation();
...
function shouldOpenBlockTypeMenuFromShortTextTap(host, pointerType, sourceKind) {
  return sourceKind === "text" && host.isMobileDragModeActiveForPointer(pointerType);
}
...
var PipelineAdapter = class {
  ...
  beginRangeSelectionSession(source, e, handle, options) {
    ...
    if (pointerType !== "mouse" && allowSecondaryDrag) {
      ...
      }, (_g = (_f = (_e = this.deps).getMobileDragLongPressMs) == null ? void 0 : _f.call(_e)) != null ? _g : MOBILE_DRAG_LONG_PRESS_MS);
    }
    ...
    if (e.pointerType !== "mouse" && ((_h = this.rangePointerSession) == null ? void 0 : _h.isIntercepting)) {
      this.mobile.applyInputGuardMode(INPUT_GUARD_MOBILE_SELECTION_GESTURE, e.target);
    }
  }
  ...
  beginPressPendingDrag(source, e, options) {
    ...
    const longPressMs = (_h = options == null ? void 0 : options.longPressMs) != null ? _h : pointerType === "mouse" ? (_d = (_c = (_b = this.deps).getMouseRangeSelectLongPressMs) == null ? void 0 : _c.call(_b)) != null ? _d : MOUSE_RANGE_SELECT_LONG_PRESS_MS : (_g = (_f = (_e = this.deps).getMobileDragLongPressMs) == null ? void 0 : _f.call(_e)) != null ? _g : MOBILE_DRAG_LONG_PRESS_MS;
    const timeoutId = skipLongPress ? null : window.setTimeout(() => this.markPressReady(sessionId, e.pointerId, pointerType), longPressMs);
    const startMoveThresholdPx = skipLongPress ? 2 : pointerType === "mouse" ? 4 : 8;
    this.pressSession = {
      sessionId,
      pointerId: e.pointerId,
      startX: e.clientX,
  ...
  retargetMobileRangeSelection(e) {
    const state = this.rangePointerSession;
    if (this.pipelineState.type !== "selecting" || !state || state.pointerType === "mouse")
      return;
  ...
  canStartDragForPointer(pointerType, source = "handle") {
    var _a, _b;
    if (source === "command")
      return true;
    if (pointerType === "mouse")
  ...
  isMobileDragModeActiveForPointer(pointerType) {
    var _a, _b;
    if (pointerType === "mouse")
      return false;
    if (!this.mobile.isMobileEnvironment())
  ...
  guardDepsForSource(source, pointerType) {
...
function normalizeBlockSelectionVisualStyle(value) {
...
function normalizeMobileDragModeToggleLocations(value) {
  if (!Array.isArray(value))
    return [...DEFAULT_SETTINGS.mobileDragModeToggleLocations];
  const locations = value.filter((entry) => entry === "view-action" || entry === "toolbar-command");
...
var DragNDropSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  displayAppearance(containerEl, i) {
    new import_obsidian2.Setting(containerEl).setName(i.headingHandle).setHeading();
    ...
    this.addNumericSetting(containerEl, {
      ...
      min: MIN_HANDLE_SIZE_PX,
      max: MAX_HANDLE_SIZE_PX,
      step: 2,
    ...
    new import_obsidian2.Setting(containerEl).setName(i.headingHighlight).setHeading();
    new import_obsidian2.Setting(containerEl).setName(i.selectionVisualStyle).setDesc(i.selectionVisualStyleDesc).addDropdown((dropdown) => dropdown.addOption("outline", i.optionBlockSelectionVisualOutline).addOption("subtle", i.optionBlockSelectionVisualSubtle).addOption("filled", i.optionBlockSelectionVisualFilled).setValue(this.plugin.settings.selectionVisualStyle).onChange(async (value) => {
      this.plugin.settings.selectionVisualStyle = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName(i.enableBlockSelectionHighlight).setDesc(i.enableBlockSelectionHighlightDesc).addToggle((toggle) => toggle.setValue(this.plugin.settings.enableBlockSelectionHighlight).onChange(async (value) => {
  ...
  displayBehavior(containerEl, i) {
    ...
    this.addNumericSetting(containerEl, {
      ...
      min: MIN_MULTI_LINE_SELECTION_LONG_PRESS_MS,
      max: MAX_MULTI_LINE_SELECTION_LONG_PRESS_MS,
      step: 50,
    ...
    new import_obsidian2.Setting(containerEl).setName(i.headingMobile).setHeading();
    if (!isMobile) {
      new import_obsidian2.Setting(containerEl).setDesc(i.mobileOnlyNotice);
    }
    new import_obsidian2.Setting(containerEl).setName(i.mobileTextLongPressDrag).setDesc(i.mobileTextLongPressDragDesc).setClass(disabledCls).addToggle((toggle) => toggle.setValue(this.plugin.settings.enableMobileTextLongPressDrag).setDisabled(!isMobile).onChange(async (value) => {
      this.plugin.settings.enableMobileTextLongPressDrag = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    if (this.plugin.settings.enableMobileTextLongPressDrag && isMobile) {
      new import_obsidian2.Setting(containerEl).setName(i.mobileDragModeToggleLocations).setDesc(i.mobileDragModeToggleLocationsDesc);
      ...
      new import_obsidian2.Setting(containerEl).setName(i.optionMobileDragModeToggleViewAction).addToggle((toggle) => toggle.setValue(this.plugin.settings.mobileDragModeToggleLocations.includes("view-action")).onChange((value) => toggleLocation("view-action", value)));
      new import_obsidian2.Setting(containerEl).setName(i.optionMobileDragModeToggleToolbarCommand).addToggle((toggle) => toggle.setValue(this.plugin.settings.mobileDragModeToggleLocations.includes("toolbar-command")).onChange((value) => toggleLocation("toolbar-command", value)));
    }
    new import_obsidian2.Setting(containerEl).setName(i.disableMobileDragModeAfterDrop).setDesc(i.disableMobileDragModeAfterDropDesc).setClass(disabledCls).addToggle((toggle) => toggle.setValue(this.plugin.settings.disableMobileDragModeAfterDrop).setDisabled(!isMobile).onChange(async (value) => {
  ...
  addNumericSetting(containerEl, opts) {
    ...
    new import_obsidian2.Setting(containerEl).setName(opts.name).setDesc(opts.desc).addSlider((slider) => slider.setLimits(opts.min, opts.max, opts.step).setValue(opts.value).onChange(async (value) => {
      currentValue = value;
...
function openNestedMenuPage(parent, view, group) {
  parent.hide();
  const child = new import_obsidian3.Menu();
...
function isMobileMenuEnvironment() {
...
async function executeMenuAction(menu, action) {
  ...
  if (!ok) {
    new import_obsidian3.Notice(action.failureNotice);
    return;
  }
  menu.hide();
}
  ...
  plugin.addCommand({
    ...
    checkCallback: (checking) => {
      if (!import_obsidian5.Platform.isMobile)
        return false;
      if (!plugin.isMobileDragModeToolbarCommandEnabled())
        return false;
      ...
      if (!checking) {
        ...
        new import_obsidian5.Notice(enabled ? "Drag mode enabled." : "Drag mode disabled.");
      }
      return true;
    }
  });
...
var DragNDropPlugin = class extends import_obsidian6.Plugin {
  ...
  async loadSettings() {
    ...
    const savedRecord = saved != null ? saved : {};
    ...
    if ("alwaysShowHandles" in savedRecord && !("handleVisibility" in savedRecord)) {
      this.settings.handleVisibility = savedRecord.alwaysShowHandles ? "always" : "hover";
    }
    ...
    delete this.settings.requireMobileDragMode;
  ...
  applySettings() {
    ...
    body.classList.toggle("dnd-mobile-handles-hidden", import_obsidian6.Platform.isMobile && !this.settings.enableMobileTextLongPressDrag);
  ...
  isMobileDragModeToolbarCommandEnabled() {
  ...
  syncMobileDragModeActionEl(actionEl) {
    ...
    (0, import_obsidian6.setIcon)(actionEl, this.getMobileDragModeActionIcon());
